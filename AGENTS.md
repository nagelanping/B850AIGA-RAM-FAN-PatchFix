...
- `archive/` — 归档历史版本的 `LOG.md`、`WORKFLOW.md` 和历史工作记录。
- `patch/linux/` —  Linux Rust 服务、systemd unit 和开发文档。
- `patch/windows/` — Windows KMDF 驱动和 Windows Service。
- `release/` — 手动发布包。
- `ref/` — 只读逆向参考：
...
- **实验批准**：于 2026-09-05 批准在本目标机承担共享端口风险的受控实验例外，允许驱动内部对固定白名单端口做**只读身份探针**；同日批准 **§5.2 第 2 步受控 SMBus 读取试验**（驱动可对白名单基址 `0xb00` 的事务寄存器执行 SPD word-read 读取 DIMM，仅读数据）与 **§5.2 第 3 步单次写回**（NCT 端口 `0x295/0x296`，仅写 page `0x0c`/reg `0x36` Virtual_TEMP 并读回校验，页保存/恢复）。批准只覆盖受控实验，不自动变成发布授权。
- **发布批准**：2026-09-06 定案：测试签名版即最终公测交付形态，项目不计划走 Microsoft Attestation/WHQL/EV 官方签名认证。公测版面向能接受关闭 Secure Boot/testsigning/内存完整性的高级用户自行安装，不做正式签名的宣传或承诺。

受控模型硬约束：

- 端口白名单固定为 `0xb00`（SMBus）、`0x295/0x296`（NCT 自定义 SIO）、`0x2e/0x2f`（标准 SIO，仅用于 NCT chip-id 身份探针，解锁-读-锁定一次完成）；不提供任意端口或任意寄存器 IOCTL。
- 公测版交付形态为测试签名版（机主 2026-09-06 定案）：安装脚本自动开启 testsigning、关闭内存完整性、导入测试证书；Secure Boot 由用户在 BIOS 自行关闭，脚本只检测提示。测试签名版仅面向能接受相应安全降级的用户；不得伪装为官方签名。不提供任意端口/寄存器 IOCTL，不以自声明端口作为访问依据。
...
- **方案 B（2026-09-06 决定，取代早期"已装映射收紧"口径）**：每轮全读 4 槽，有效温度 1..120°C（0°C 屏蔽），单槽瞬时垃圾/超时同轮重试一次，无任一有效槽才整轮不写。空槽偶发杂散数据无法污染判断（无跨轮状态）。身份门禁 60s 缓存心跳（QUERY_HW 诊断不缓存）。

### 服务职责

- 服务负责 SCM 生命周期、0.5 秒调度、有限退避、`--once`、日志和错误重试。
- 公测版服务使用 AUTO_START（开机自启，机主 2026-09-06 定案）；开发阶段脚本 `identity-gate-prep.ps1` 仍用 DEMAND_START。
...
- 已授权 agent 在目标机直接执行签名/加载/验证（管理员权限足够；本会话已多次实机执行）。需要重启时提前告知机主，重启后继续。
...
1. 开始任务前阅读根目录 `LOG.md`、`WORKFLOW.md`，避免重复调查。
