...
- **实验批准**：机主于 2026-09-05 批准在本目标机承担共享端口风险的受控实验例外，允许驱动内部对固定白名单端口做**只读身份探针**；同日批准 **§5.2 第 2 步受控 SMBus 读取试验**（驱动可对白名单基址 `0xb00` 的事务寄存器执行 SPD word-read 读取 DIMM，仅读数据）与 **§5.2 第 3 步单次写回**（NCT 端口 `0x295/0x296`，仅写 page `0x0c`/reg `0x36` Virtual_TEMP 并读回校验，页保存/恢复）。批准只覆盖受控实验，不自动变成发布授权。
...
- **方案 B（2026-09-06 机主决定，取代早期"已装映射收紧"口径）**：每轮全读 4 槽，有效温度 1..120°C（0°C 屏蔽），单槽瞬时垃圾/超时同轮重试一次，无任一有效槽才整轮不写。空槽偶发杂散数据无法污染判断（无跨轮状态）。身份门禁 60s 缓存心跳（QUERY_HW 诊断不缓存）。
...
4. ✅ 单次写回（已批准并实机验证，2026-09-05）：恢复 `FEED_ONCE`，一次完成全读取→校验→最高温→NCT page `0x0c`/reg `0x36` 写回并读回校验，页保存/恢复。实机 6 轮 `written=35 readback=35 Status=0`。
5. ✅ 常驻服务（已批准并实机验证，2026-09-05；方案 B 2026-09-06）：0.5 秒调度 FEED_ONCE + SCM 生命周期（DEMAND_START）；2026-09-06 起语义为方案 B（温度屏蔽 1..120 + 单槽重试 + 门禁 60s 缓存）。实机压测 45-46°C 稳定写回一致、零 WARN/FATAL。睡眠/重启自动恢复与动态温升由机主在测试版本自验。
...
- 机主已授权 agent 在目标机直接执行签名/加载/验证（管理员权限足够；本会话已多次实机执行）。需要重启时提前告知机主，重启后继续。
