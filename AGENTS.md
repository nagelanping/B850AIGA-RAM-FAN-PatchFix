...
4. ✅ 单次写回（已批准并实机验证，2026-09-05）：恢复 `FEED_ONCE`，一次完成全读取→校验→最高温→NCT page `0x0c`/reg `0x36` 写回并读回校验，页保存/恢复。实机 6 轮 `written=35 readback=35 Status=0`。已知局限（M1-1）：BUS_ERR 当空槽忽略，常驻阶段建“已装地址映射”后收紧。
5. ✅ 常驻服务（已批准并实机验证，2026-09-05）：0.5 秒调度 FEED_ONCE + SCM 生命周期（DEMAND_START）+ 驱动内已装映射（M1-1）；实机验证 0.5s 写回一致、服务停止保留值、重启平滑。睡眠/重启自动恢复与动态温升由机主在测试版本自验。
