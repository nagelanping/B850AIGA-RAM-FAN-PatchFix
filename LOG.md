...
- **当前阶段**：§5.2 第 1 步身份门禁、第 2 步受控 SMBus 读取均已实机验证通过（已装 `0x53`/`0x51`，空槽 `0x52`/`0x50`）。机主 2026-09-05 批准第 3 步单次写回；`FEED_ONCE` 已实现（身份门禁→全读→校验→最高温→NCT page `0x0c`/reg `0x36` 写回→读回校验，页保存/恢复），构建与自检通过、子代理审查无阻塞项，待实机 `--once` 验证（写入值应使 pwm5/转速按 LOG 实测响应变化）。
## 根因与目标链路
