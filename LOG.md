...
## 2026-09-05 机主批准 §5.2 第 3 步单次写回，实机验证通过（FEED_ONCE）

- **批准**：机主批准 §5.2 第 3 步单次写回（FEED_ONCE：读取→校验→最高温→NCT page `0x0c`/reg `0x36` 写回并读回校验）。写 SIO 寄存器可逆、重启可由 BIOS 恢复，属非破坏性；不修改曲线/温度源/固件。
- **驱动**（commit `bbe11d3`）：`hw.c` 新增 `RamFanNctWriteVirtTemp`——端口固定白名单 `0x295/0x296`，写前保存当前页、选页 `0x0c` 保留高 4 位（`(v&0xf0)|page`）、写 reg `0x36`、重选页读回校验、成功或失败路径都尽力恢复原页；只写该寄存器。`ramfan.c` 新增 `RamFanFeedOnce`——身份门禁不匹配 Status=3 零事务；读全部槽；TIMEOUT/BAD_DATA 任一槽→Status=1 整体失败不写（不降速）；无成功槽→Status=1；写回/读回不一致→Status=2；成功 Status=0。服务 `--once` 启用（返回 0=成功/1=硬件或 I/O 失败/2=参数错误）；`--install`/`--uninstall` 仍阻断。
- **子代理独立审查**：无 S1。M1-1 记录为常驻阶段收紧项——BUS_ERR 无法区分空槽与已装槽 CRC（LOG 已确认 0x04 不可区分），本函数把 BUS_ERR 当空槽忽略；本机实测已装恒 OK、空槽恒 BUS_ERR，风险边界明确，常驻阶段须建“已装地址映射”后把映射内 BUS_ERR 判为整体失败。L1 修复：删除死变量 `g_FeedDisabled`、死宏 `RAMFAN_FEED_HW_UNAVAILABLE`（改注释保留）、安装文案/ioctl 头注释同步。
...
## 2026-09-06 压测暴露假失败，方案 B + 重试 + 门禁缓存（commits e46170c/03b12d1/4b8466e）

- **问题**：常驻压测升温时出现间歇/持续 `FEED status=1`。停压测后 `--once` 两已装槽（0x53/0x51）均 OK 仍失败 → 定位为程序缺陷而非硬件：旧 ae32c06 逻辑把“曾 OK”永久记入 InstalledMask 且每轮全读空槽；空槽在 SMBus 争用下偶发读到无 0x04 位的杂散数据，0°C 被 0..120 判 OK → 空槽永久入 mask，其后正常 BUS_ERR(0x06) 命中“已装槽异常→整轮失败”。
- **决策（机主 2026-09-06）**：方案 B——去掉映射状态机与跨 IOCTL 状态，每轮全读 4 槽；有效温度 1..120°C（0°C 属明显异常屏蔽，DDR 运行中不可能）；只取 OK 槽最高有效温度写入。
- **偶发根因（槽级诊断）**：压测下已装槽偶发读到 raw=0xffxx 垃圾/hst 无数据（FCH SMBus 多主机仲裁特征，非 DIMM 故障），换算越界判 BAD_DATA。下一轮自愈，但每约 1 分钟一次单轮空档。
- **决策（机主）**：① 单槽 BAD_DATA(含 0°C)/TIMEOUT 同轮立即重试一次（BUS_ERR 空槽 NACK 不重试防徒增流量）；② 身份门禁 60s 缓存心跳（M1-5 折中）：FEED 高频路径每 60s 重跑一次注册表枚举+SIO 探针，QUERY_HW 诊断不缓存。0.5s 轮询下偶发空档最多延迟 1s 温升，几乎不可感知。
- **子代理审查**：无 S1。修复 M1-1（ZwEnumerateKey 输入/输出长度同址复用→分离 retLen）、M1-2（NameLength USHORT 截断防御）、M1-3（InstallService 假成功/描述未生效/句柄泄漏）、M1-4（FATAL 日志丢 %s 句）。最终审查（HEAD 4b8466e）确认无 S1 可发布受控测试版。
