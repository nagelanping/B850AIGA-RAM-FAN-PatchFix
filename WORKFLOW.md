...
- 2026-09-05：§5.2 第 1 步**只读身份门禁实机验证通过**（`HwMatched=1`）；**机主批准 §5.2 第 2 步受控 SMBus 读取试验**，`READ_DIMM_TEMP` 已实现并通过构建/自检/子代理审查，待实机 `--dimm` 确认槽语义。
- 仍阻断：`FEED_ONCE`（写 NCT，§5.2 第 3 步）需另一次批准。
## 4. 修复优先级与路线选择
...
1. ✅ 2026-09-05：§5.2 第 1 步身份门禁已实机验证通过；机主已批准 §5.2 第 2 步受控 SMBus 读取试验，`READ_DIMM_TEMP` 已实现（构建/自检/子代理审查通过）。
2. 实机执行 `identity-gate-prep.ps1` 加载驱动后运行 `ramfan-service.exe --dimm`：记录 4 槽状态（已装 `0x53/0x52` 应 OK 有温度，空槽应呈现 BUS_ERR+HstSts 0x06），结果写回 `LOG.md`。
3. 槽语义确认后，经机主批准与独立审查再进入 §5.2 第 3 步（单次写回 `FEED_ONCE`）。
