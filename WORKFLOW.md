...
- 2026-09-05：§5.2 第 1 步只读身份门禁实机验证通过（`HwMatched=1`）；§5.2 第 2 步受控 SMBus 读取试验实机完成——已装 DIMM `0x53`/`0x51`（36/33°C），空槽 `0x50`/`0x52` 呈 BUS_ERR+HstSts 0x06，6 轮重复稳定。
...
1. ✅ 2026-09-05：§5.2 第 1 步身份门禁与第 2 步受控 SMBus 读取均已实机验证通过（结果见 LOG.md）。
2. 机主批准 §5.2 第 3 步单次写回后：恢复 `FEED_ONCE`（全读取→最高温→NCT page `0x0c`/reg `0x36` 写回并读回校验），独立审查后实机验证。
3. 写回验证通过后再进入动态闭环与 0.5s 常驻服务（需再次批准）。
