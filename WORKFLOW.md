...
1. ✅ 2026-09-05：§5.2 第 1/2/3 步均已实机验证通过（身份门禁→SMBus 读取→FEED_ONCE 单次写回，见 LOG.md）。
2. 机主批准常驻服务阶段后：驱动内实现“已装地址映射”（首轮建映射、映射内 BUS_ERR 判整体失败，M1-1），服务改 0.5s FEED_ONCE 循环 + SCM 生命周期（DEMAND_START，验收后才自动）；独立审查后实机验证睡眠恢复与重启恢复。
3. 动态温升联动（温度变化→转速响应）在常驻阶段一并验收。
