...
- 2026-09-05：§5.2 第 1 步只读身份门禁、第 2 步受控 SMBus 读取、第 3 步单次写回（FEED_ONCE）均实机验证通过。`--once` 6 轮 `status=0 written=35 readback=35`，NCT page `0x0c`/reg `0x36` 写回与读回校验一致。
- 仍阻断：常驻 0.5s 喂值服务（SCM 生命周期）需另一次批准；常驻阶段需在驱动内实现“已装地址映射”（M1-1）后收紧 BUS_ERR 语义。
## 4. 修复优先级与路线选择
...
1. ✅ 2026-09-05：§5.2 第 1/2/3 步与常驻喂值阶段均已实机验证通过（身份门禁→SMBus 读取→FEED_ONCE→0.5s 常驻循环，见 LOG.md）。
2. ✅ 常驻闭环（commit `ae32c06`）：驱动已装映射收紧（M1-1）+ 服务 0.5s FEED_ONCE 循环 + SCM DEMAND_START；实机验证 0.5s 周期写回一致、服务停止保留值、重启平滑。
3. 测试版本交付机主自验：动态温升联动（温度变化→转速响应）、睡眠恢复、系统重启后自动恢复（当前 DEMAND_START 需手动启动；验收通过才改自动启动）。
